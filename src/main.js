...
const stormCamera = document.querySelector('#storm-camera');
const stormBackdrop = document.querySelector('#storm-backdrop');
const stormRainNear = document.querySelector('#storm-rain-near');
const stormSpray = document.querySelector('#storm-spray');
...
const stormEntryCurrent = document.querySelector('#storm-entry-current');
...
function buildStorm() {
  ...
  stormBackdrop.fetchPriority = 'high';
  stormWorld.style.setProperty('--storm-background', `url("${stormBackgroundUrl}")`);
  stormEntryCurrent.style.setProperty('--storm-entry-background', `url("${stormBackgroundUrl}")`);
  ...
  for (let index = 0; index < rainCount; index += 1) {
    ...
    streak.style.setProperty('--rain-width', `${index % 7 === 0 ? 2 : 1}px`);
  ...
  for (let index = 0; index < sprayCount; index += 1) {
    ...
    spray.style.setProperty('--spray-y', `${4 + ((index * 31) % 88)}%`);
    spray.style.setProperty('--spray-size', `${1.5 + (index % 6) * .7}px`);
    spray.style.setProperty('--spray-delay', `${(index % 14) * -.12}s`);
    spray.style.setProperty('--spray-speed', `${1.25 + (index % 7) * .17}s`);
    spray.style.setProperty('--spray-drift', `${70 + (index % 8) * 22}px`);
...
let activeScene = 0;
let stormClickCount = 0;
let stormAwaitingClick = false;
...
stormWhaleControl.addEventListener('click', () => {
  if (activeScene !== 2 || !stormAwaitingClick || stormClickCount >= 3) return;
  stormAwaitingClick = false;
  stormClickCount += 1;
  stormWhaleControl.hidden = true;
  stormWhaleHint.hidden = true;
...
function renderStorm({ progress }) {
  ...
  stormWorld.style.setProperty('--storm-lightning-near', frame.storm.lightningNear.toFixed(4));
...
function renderFinale({ progress }) {
  ...
  // The artwork is fully composed before the interaction, but both layers stay
  // behind angular masks. The scan, rather than the automatic scene timeline,
  // is solely responsible for revealing the whale and Pastel's portrait.
  finaleWorld.style.setProperty('--memory-opacity', '1');
  finaleWorld.style.setProperty('--memory-scale', `${lerp(1.08, targetScale, cameraPullback)}`);
  if (!finaleWorld.classList.contains('is-complete')) {
...
function renderFinaleScan(state) {
  const whaleReveal = clamp(state.whale);
  const portraitReveal = clamp(state.portrait);
  ...
  finaleReveal.dataset.whaleReveal = whaleReveal.toFixed(4);
  finaleReveal.dataset.portraitReveal = portraitReveal.toFixed(4);
  finaleReveal.dataset.scanRadius = radius.toFixed(2);
}
...
function sendLightToWhale(event) {
  if (sendLight.disabled || finaleSequenceStarted) return;
  finaleSequenceStarted = true;
  sendLight.disabled = true;
  sendLight.classList.remove('is-ready');
  finaleWorld.classList.add('is-sequencing');
  ...
  finaleSequenceTimeline = gsap.timeline()
    .to(light, {
      x: centerX - startX,
      y: centerY - startY,
      scale: 1.35,
    ...
    .call(() => setFinalePhase('sweep-forward'))
    .to(scanState, {
      angle: 360,
      whale: 1,
      duration: 2.35,
    ...
    .call(() => {
      finaleReveal.classList.add('has-whale-reveal');
      setFinalePhase('sweep-reverse');
    })
    .to(scanState, {
      angle: 0,
      portrait: 1,
